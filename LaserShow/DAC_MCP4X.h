...
class MCP4X {
  public:
    ...
    void setVref(byte chan, unsigned int vref)			{
      vrefs[chan & 1] = vref;
    }
    void setVref(unsigned int vref)						{
      vrefs[0] = vrefs[1] = vref;
    }
    void setBuffer(byte chan, boolean buffered);
    void setGain2x(byte chan, boolean gain2x = 1);
    void setAutoLatch(boolean enabled = 1)				{
      autoLatch = enabled;
    }
    ...
    void output(unsigned short data) 					{
      output(MCP4X_CHAN_A, data);
    }
    ;
    /* same as output(), but having A/B makes more sense for dual DACs */
    void outputA(unsigned short data)					{
      output(MCP4X_CHAN_A, data);
    }
    void outputB(unsigned short data) 					{
      output(MCP4X_CHAN_B, data);
    }
    ...
    int getGain(byte chan)						{
      return regs[chan & 1] & MCP4X_GAIN_1X ? 1 : 2;
    }
    ...
    */
    void setAutomaticallyLatchDual(bool latch)	{
      autoLatch = latch;
    };
