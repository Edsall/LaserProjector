...
#define NUM_VALID_COLORS 7

const byte validColors[] PROGMEM = {
...
public:
  ...
  //Laser(int laserPin);
  Laser(int duo_control_pin, int laser_pin_R, int laser_pin_G, int laser_pin_B);
  ...
  void on();
  void colorOn();  // forces a color change when lasers already on
  void off();
...
private:
  ...
  int _duo_control_pin, _laser_pin_R, _laser_pin_G, _laser_pin_B;
