...
void Drawing::drawString(String text, int x, int y, int count)
    // draw a string
    // x and y is the start location
    // count is the number of times to draw at that location
{
  for (int loop = 0; loop < count; loop ++)
  {
    int i = 0;
    int x1 = x;
    while (text.charAt(i) != '\0')  // go through all letters
    {
      x1 += drawLetter(text.charAt(i), x1, y);
      i ++;  // advance to next letter
    }
  }
}
...
long Drawing::stringAdvance(String text)
    //
    // calculate how long to advance for entire string
    // by walking throught the letters
    //
{
  long adv = 0;
  int i = 0;
...
long Drawing::advance(byte letter)
    //
    // advances by the space of a letter
    // some letters are short and some are long
    //
{
  long adv = 850;
  if (letter == 'I') {
...
void Drawing::drawObject(const unsigned short* data, int size, long translateX, long translateY)
    //
    // Array of points in data
    // number of points in size
    // translateX and translateY are relative offsets to the points
    //
{
...
void Drawing::drawObjectRotated(const unsigned short* data, int size, long centerX, long centerY, int angle)
    //
    // draws object rotated
    // data is array of points
    // size is number of points
    // centerX and centerY are the coordinates to rotate around
