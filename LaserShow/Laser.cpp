...
Laser::Laser(int duo_control_pin, int laser_pin_R, int laser_pin_G, int laser_pin_B)
{
  _duo_control_pin = duo_control_pin;
  _laser_pin_R = laser_pin_R;
  _laser_pin_G = laser_pin_G;
  _laser_pin_B = laser_pin_B;
...
void Laser::init()
{
  dac.init(MCP4X_4822, 5000, 5000, DAC_SPI_CS, DAC_LATCH, 1);
  dac.setGain2x(MCP4X_CHAN_A, 0);
  dac.setGain2x(MCP4X_CHAN_B, 0);
  dac.begin(1);

  pinMode(_duo_control_pin, OUTPUT);
  pinMode(_laser_pin_R, OUTPUT);
  pinMode(_laser_pin_G, OUTPUT);
  pinMode(_laser_pin_B, OUTPUT);
  digitalWrite(_duo_control_pin, LOW);
}
...
void Laser::on()
{
  if (!_state && !_laserForceOff)
  {
    ...
    colorOn();
    // digitalWrite(_laser_pin_R, HIGH);
    // digitalWrite(_laser_pin_G, HIGH);
    // digitalWrite(_laser_pin_B, HIGH);
  }

}

void Laser::colorOn(){
    digitalWrite(_laser_pin_R, (_color & 0x1));
    digitalWrite(_laser_pin_G, (_color & 0x2));
    digitalWrite(_laser_pin_B, (_color & 0x4));
}
...
void Laser::off()
{
  if (_state)
  {
    wait(LASER_TOGGLE_DELAY);
    _state = 0;
    digitalWrite(_laser_pin_R, LOW);
    digitalWrite(_laser_pin_G, LOW);
    digitalWrite(_laser_pin_B, LOW);
  }
}

void Laser::setColor(byte color)
{
    if (_color != color) {
        _color = color;
        if (_state) {
